	...
	TreeWidget.prototype = {
		...
		initHelpdesk: function ($tree) {
			$tree.find('a.helpdesk-btn').click(function() {
				var confirmText = $(this).data('confirm-text') ?? Yii.t('helpdesk', 'Вы уверены?');
				...
				options = {
					...
					other : {
						...
						onUploadComplete:  function (file , res) {
							...
							file.hash = hash;
							$hiddenInputEl = $(options.valueElemSelector);
                            let ajaxUrl = '/pl/telegram/control/file/check-file';
                            if (window.maxEnabledFeature === true) {
                                ajaxUrl = '/pl/inbox-file/check-file';
                            }
							...
							ajaxCall(ajaxUrl,
								...
								function (response) {
									if (response.success === true) {
										...
										var type_block =
											$('<span>'+Yii.t('widgets', 'Отправить файл как')+': ' +
